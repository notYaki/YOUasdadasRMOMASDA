  ...
  run: async (client, message, args) => {
      ...
      if(args.join(" ").includes("deezer")){
        ...
        deezer.playlist.tracks(track).then(async function(result) {
         ...
         /* if(tracklength > 25) 
          {
              message.reply("the current maximum of tracks for deezer playlists are 25 tracks, if you wanna use bigger playlists, then dm `Ÿακι ジ#4635`\n\nI will use the first 25 songs!"); 
              tracklength = 25;
          } */
        ...
        if(tracklength > 25) 
        {
            message.reply("the current maximum of tracks for spotify playlists are 25 tracks, if you wanna use bigger playlists, then dm `Ÿακι ジ#4635`\n\nI will use the first 25 songs!"); 
            tracklength = 25;
        }
        functions.embedbuilder(client, 5000, message, config.colors.yes, "<:youtube:769675858431705109> Fetching the songs!", "This will take me around about: " + tracklength/2 + " seconds");
        for(let i = 0; i < items.length; i++){
