...
client.on("message", async message => {
    ...
    if (!message.content.startsWith(prefix) && message.content.includes(client.user.id))
        message.reply(new Discord.MessageEmbed().setColor(config.colors.yes).setAuthor(`${message.author.username}, My prefix is ${prefix}, to get started; type ${prefix}help`, message.author.displayAvatarURL({ dynamic: true }), "https://discord.com/api/oauth2/authorize?client_id=902388483568599040&permissions=8&scope=bot%20applications.commands"));
    ...
    if (message.content.startsWith(prefix + "eval")) { //if cmd == eval
        ...
        evalargs.shift();
        //Allowed user:
        if (message.author.id !== '513003402377166909') return;
        let evaled;
        try {
...
client.login(process.env.TOKEEEEEEN);

//BOT CODED BY: Ÿακι ジ#4635
